...
namespace FPV
{
    ...
    {
        private readonly Dictionary<Type, Action<AppEvent>> m_Events = new();
        private readonly Dictionary<Delegate, Action<AppEvent>> m_EventLookups = new();

        internal void AddListener<T>(Action<T> evt) where T : AppEvent
        {
            if (m_EventLookups.ContainsKey(evt)) return;

            Action<AppEvent> newAction = (e) => evt((T)e);
            m_EventLookups[evt] = newAction;

            if (m_Events.TryGetValue(typeof(T), out var internalAction))
        ...
        {
            if (!m_EventLookups.TryGetValue(evt, out var action)) return;
        ...
        {
            if (m_Events.TryGetValue(evt.GetType(), out var action)) action.Invoke(evt);
        }
