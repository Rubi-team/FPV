...
namespace FPV.Editor
{
    ...
    public static class UIElementsUtils
    {
        ...
        {
            var path = $"{Path.Combine(k_UIFilesPathInTemplate, fileName)}.uxml";
        ...
        public static Button SetupButton(string buttonName, Action onClickAction, bool isEnabled, VisualElement parent,
            string text = "", string tooltip = "", bool showIfEnabled = true)
        {
            ...
            if (showIfEnabled && isEnabled) Show(button);
        ...
        public static EnumField SetupEnumField<T>(string enumName, string text,
            EventCallback<ChangeEvent<Enum>> onValueChanged, VisualElement parent, T defaultValue) where T : Enum
        {
            var uxmlField = parent.Q<EnumField>(enumName);
        ...
        public static Toggle SetupToggle(string name, string label, string text, bool defaultValue,
            EventCallback<ChangeEvent<bool>> onValueChanged, VisualElement parent)
        {
            var uxmlField = parent.Q<Toggle>(name);
        ...
        public static IntegerField SetupIntegerField(string name, int value,
            EventCallback<ChangeEvent<int>> onValueChanged, VisualElement parent)
        {
            var uxmlField = parent.Q<IntegerField>(name);
        ...
        public static TextField SetupStringField(string name, string label, string value,
            EventCallback<ChangeEvent<string>> onValueChanged, VisualElement parent)
        {
            var uxmlField = parent.Q<TextField>(name);
            uxmlField.label = label;
            uxmlField.value = value;
            uxmlField.SetEnabled(true);
            uxmlField.RegisterCallback(onValueChanged);
