...
namespace CodeMonkey.Toolkit {
    ...
    [InitializeOnLoad]
    public class MainWindowToolkit : EditorWindow {


        [SerializeField] private CodeMonkeyToolkitSO codeMonkeyToolkitSO;
        ...
        [SerializeField] private VisualTreeAsset toolListEntryVisualTreeAsset;
        [SerializeField] private VisualTreeAsset toolTagSingleVisualTreeAsset;


        static MainWindowToolkit() {
        ...
        private static void Startup() {
            ...
            try {
                CodeMonkeyToolkitSO codeMonkeyInteractiveSO = CodeMonkeyToolkitSO.GetCodeMonkeyToolkitSO();
        ...
        private VisualElement toolListVisualElement;
        private VisualElement toolSingleVisualElement;
        private VisualElement mainMenuVisualElement;
        private ToolSO toolSO;
        private Vector2 lastListScrollPosition;
        private SubWindow lastSubWindow;


        [MenuItem("Tools/Code Monkey Toolkit", priority = 0)]
        public static void ShowWindow() {
            try {
                MainWindowToolkit window = GetWindow<MainWindowToolkit>();
                window.titleContent = new GUIContent("Code Monkey Toolkit");
            } catch (Exception e) {
                Debug.LogError(e);
            }
        }
        ...
        private SubWindow GetActiveSubWindow() {
            if (toolListVisualElement.style.display == DisplayStyle.Flex) {
                return SubWindow.ToolList;
            }
            if (toolSingleVisualElement.style.display == DisplayStyle.Flex) {
                return SubWindow.ToolSingle;
            }
        ...
        public void CreateGUI() {
            ...
            toolListVisualElement = root.Q<VisualElement>("toolList");
            toolSingleVisualElement = root.Q<VisualElement>("toolSingle");

            root.Q<Label>("versionLabel").text = CodeMonkeyToolkitSO.GetCodeMonkeyToolkitSO().currentVersion;

            Button toolListButton = mainMenuVisualElement.Q<Button>("toolListButton");
            toolListButton.RegisterCallback((ClickEvent clickEvent) => {
                ShowToolList();
            });

            CreateToolList(null);

            Button toolListBackToMainMenuButton = toolListVisualElement.Q<Button>("backToMainMenuButton");
            toolListBackToMainMenuButton.RegisterCallback((ClickEvent clickEvent) => {
                ShowMainMenu();
            });

            TextField searchTextField = toolListVisualElement.Q<TextField>("searchTextField");
            searchTextField.RegisterCallback((ChangeEvent<string> changeEvent) => {
                CreateToolList(changeEvent.newValue);
            });

            // Filter Preset Buttons
            toolListVisualElement.Q<Button>("utilityButton").RegisterCallback((ClickEvent clickEvent) => {
                searchTextField.value = "Utility";
            });
            toolListVisualElement.Q<Button>("uiButton").RegisterCallback((ClickEvent clickEvent) => {
                searchTextField.value = "UI";
            });
            toolListVisualElement.Q<Button>("systemButton").RegisterCallback((ClickEvent clickEvent) => {
                searchTextField.value = "System";
            });
            toolListVisualElement.Q<Button>("mechanicButton").RegisterCallback((ClickEvent clickEvent) => {
                searchTextField.value = "Mechanic";
            });
            toolListVisualElement.Q<Button>("controllerButton").RegisterCallback((ClickEvent clickEvent) => {
                searchTextField.value = "Controller";
            });
            toolListVisualElement.Q<Button>("polishButton").RegisterCallback((ClickEvent clickEvent) => {
                searchTextField.value = "Polish";
            });
            toolListVisualElement.Q<Button>("gridButton").RegisterCallback((ClickEvent clickEvent) => {
                searchTextField.value = "Grid";
            });
            toolListVisualElement.Q<Button>("shaderButton").RegisterCallback((ClickEvent clickEvent) => {
                searchTextField.value = "Shader";
            });

            Button backToToolListButton = toolSingleVisualElement.Q<Button>("backToToolListButton");
            backToToolListButton.RegisterCallback((ClickEvent clickEvent) => {
                ShowToolList();
            });

            // ToolSingle menu
            Button openDemoButton = toolSingleVisualElement.Q<Button>("openDemoButton");
            openDemoButton.RegisterCallback((ClickEvent clickEvent) => {
                if (toolSO == null) return;
                EditorSceneManager.OpenScene(AssetDatabase.GetAssetPath(toolSO.demoScene));
                EditorGUIUtility.PingObject(toolSO.demoScene);
            });
            Button pingButton = toolSingleVisualElement.Q<Button>("pingButton");
            pingButton.RegisterCallback((ClickEvent clickEvent) => {
                if (toolSO == null) return;
                EditorGUIUtility.PingObject(toolSO.demoScene);
            });

            Button walkthroughVideoButton = toolSingleVisualElement.Q<Button>("walkthroughVideoButton");
            walkthroughVideoButton.RegisterCallbackOnce((ClickEvent clickEvent) => {
                if (toolSO == null) return;
                string walkthroughVideoUrl = $"https://www.youtube.com/watch?v={toolSO.walkthroughVideoYouTubeVideo.youTubeId}";
                Application.OpenURL(walkthroughVideoUrl);
            });

            Button tutorialVideoButton = toolSingleVisualElement.Q<Button>("tutorialVideoButton");
            tutorialVideoButton.RegisterCallbackOnce((ClickEvent clickEvent) => {
                if (toolSO == null) return;
                string tutorialVideoUrl = $"https://unitycodemonkey.com/video.php?v={toolSO.tutorialVideoYouTubeVideo.youTubeId}";
                Application.OpenURL(tutorialVideoUrl);
            });


            switch (lastSubWindow) {
                case SubWindow.MainMenu:
                    ShowMainMenu();
                    break;
                case SubWindow.ToolList:
                    ShowToolList();
                    break;
                case SubWindow.ToolSingle:
                    ShowTool(toolSO);
                    break;
            }
        }

        private void CreateToolList(string filter) {
            List<ToolSO> toolSOList;
            if (string.IsNullOrEmpty(filter)) {
                toolSOList = codeMonkeyToolkitSO.toolSOList;
            } else {
                toolSOList = codeMonkeyToolkitSO.GetFilteredToolSOList(filter);
            }

            // Setup Tool List
            ScrollView toolListScrollView = toolListVisualElement.Q<ScrollView>();

            DestroyChildren(toolListScrollView);

            foreach (ToolSO toolSO in toolSOList) {
                VisualElement toolListEntryVisualElement = toolListEntryVisualTreeAsset.Instantiate();

                Label titleLabel = toolListEntryVisualElement.Q<Label>("titleLabel");
                titleLabel.text = toolSO.nameString;

                Label descriptionLabel = toolListEntryVisualElement.Q<Label>("descriptionLabel");
                descriptionLabel.text = toolSO.description;

                VisualElement iconVisualElement = toolListEntryVisualElement.Q<VisualElement>("icon");
                iconVisualElement.style.backgroundImage = new StyleBackground(toolSO.iconSprite);

                ToolSO thisToolSO = toolSO;
                toolListEntryVisualElement.RegisterCallback((ClickEvent clickEvent) => {
                    lastListScrollPosition = toolListScrollView.scrollOffset;
                    ShowTool(thisToolSO);
                });

                VisualElement tagsContainerVisualElement = toolListEntryVisualElement.Q<VisualElement>("tagsContainer");
                tagsContainerVisualElement.Clear();

                string[] tagArray = toolSO.GetTagArray();

                foreach (string tag in tagArray) {
                    VisualElement toolTagSingleVisualElement = toolTagSingleVisualTreeAsset.Instantiate();

                    Label textLabel = toolTagSingleVisualElement.Q<Label>("textLabel");
                    textLabel.text = tag;

                    tagsContainerVisualElement.Add(toolTagSingleVisualElement);
                }

                toolListScrollView.Add(toolListEntryVisualElement);
            }
        }

        private void ShowToolList() {
            toolListVisualElement.style.display = DisplayStyle.Flex;
            toolSingleVisualElement.style.display = DisplayStyle.None;
            mainMenuVisualElement.style.display = DisplayStyle.None;

            ScrollView toolListScrollView = toolListVisualElement.Q<ScrollView>();
            toolListScrollView.scrollOffset = lastListScrollPosition;
            toolListScrollView.schedule.Execute(() => {
                toolListScrollView.scrollOffset = lastListScrollPosition;
            });

            lastSubWindow = SubWindow.ToolList;
        }

        private void ShowTool(ToolSO toolSO) {
            this.toolSO = toolSO;

            lastSubWindow = SubWindow.ToolSingle;

            toolListVisualElement.style.display = DisplayStyle.None;
            toolSingleVisualElement.style.display = DisplayStyle.Flex;
            mainMenuVisualElement.style.display = DisplayStyle.None;

            Label titleLabel = toolSingleVisualElement.Q<Label>("titleLabel");
            Label descriptionLabel = toolSingleVisualElement.Q<Label>("descriptionLabel");

            titleLabel.text = toolSO.nameString;
            descriptionLabel.text = toolSO.description;

            VisualElement iconVisualElement = toolSingleVisualElement.Q<VisualElement>("icon");
            iconVisualElement.style.backgroundImage = new StyleBackground(toolSO.iconSprite);


            VisualElement tutorialVideoContainerVisualElement = toolSingleVisualElement.Q<VisualElement>("tutorialVideoContainer");
            tutorialVideoContainerVisualElement.Clear();

            if (toolSO.tutorialVideoYouTubeVideo.youTubeId != "") {
                string thumbnailUrl = $"https://img.youtube.com/vi/{toolSO.tutorialVideoYouTubeVideo.youTubeId}/mqdefault.jpg";
                string tutorialVideoUrl = $"https://unitycodemonkey.com/video.php?v={toolSO.tutorialVideoYouTubeVideo.youTubeId}";
                AddVideoReference(
                    videoTemplateVisualTreeAsset,
                    tutorialVideoContainerVisualElement,
                    thumbnailUrl,
                    toolSO.tutorialVideoYouTubeVideo.title,
                    tutorialVideoUrl,
            ...
            Button tutorialVideoButton = toolSingleVisualElement.Q<Button>("tutorialVideoButton");
            Label noteLabel = toolSingleVisualElement.Q<Label>("noteLabel");
            if (toolSO.tutorialVideoYouTubeVideo.youTubeId == "") {
                tutorialVideoButton.style.display = DisplayStyle.None;
                noteLabel.style.display = DisplayStyle.None;
            } else {
                tutorialVideoButton.style.display = DisplayStyle.Flex;
                noteLabel.style.display = DisplayStyle.Flex;
            }

            // Show Related
            VisualElement relatedVideosVisualElement = toolSingleVisualElement.Q<VisualElement>("relatedVideos");

            if (toolSO.relatedVideoYouTubeVideoArray != null && toolSO.relatedVideoYouTubeVideoArray.Length > 0) {
                // Has related videos
                relatedVideosVisualElement.style.display = DisplayStyle.Flex;
                VisualElement relatedVideosContainerVisualElement = toolSingleVisualElement.Q<VisualElement>("relatedVideosContainer");

                relatedVideosContainerVisualElement.Clear();

                foreach (YouTubeVideo youTubeVideo in toolSO.relatedVideoYouTubeVideoArray) {
                    AddRelatedVideo(youTubeVideo, relatedVideosContainerVisualElement);

                    void AddRelatedVideo(YouTubeVideo youTubeVideo, VisualElement containerVisualElement) {
                        string thumbnailUrl = $"https://img.youtube.com/vi/{youTubeVideo.youTubeId}/mqdefault.jpg";
                        string url = $"https://unitycodemonkey.com/video.php?v={youTubeVideo.youTubeId}";
                        AddVideoReference(
                            videoTemplateVisualTreeAsset,
                ...
                // Does not have related videos
                relatedVideosVisualElement.style.display = DisplayStyle.None;
            }
        }
        ...
        private void ShowMainMenu() {
            ...
            lastSubWindow = SubWindow.MainMenu;

            // Check for updates
            CodeMonkeyToolkitSO.CheckForUpdates((CodeMonkeyToolkitSO.LastUpdateResponse lastUpdateResponse) => {
                if (!lastUpdateResponse.enabled || codeMonkeyToolkitSO.currentVersion == lastUpdateResponse.version) {
                ...
                textLabel.text = "New version available!\n" +
                    codeMonkeyToolkitSO.currentVersion + " -> " + lastUpdateResponse.version + "\n" +
                    "<u>Click here!</u>";
            ...
            CodeMonkeyToolkitSO.GetLatestMessage((CodeMonkeyToolkitSO.WebsiteLatestMessage websiteLatestMessage) => {
            ...
            CodeMonkeyToolkitSO.GetLastQOTD((CodeMonkeyToolkitSO.LastQOTDResponse lastQOTDResponse) => {
                openQotdURL = () => {
            ...
            CodeMonkeyToolkitSO.GetWebsiteLatestVideos((CodeMonkeyToolkitSO.LatestVideos latestVideos) => {
            ...
            void AddLatestVideoReference(CodeMonkeyToolkitSO.LatestVideoSingle latestVideoSingle, VisualElement containerVisualElement) {
            ...
            CodeMonkeyToolkitSO.GetLastDynamicHeader((CodeMonkeyToolkitSO.LastDynamicHeaderResponse lastDynamicHeaderResponse) => {
