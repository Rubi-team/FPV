...
namespace FMODUnity
{
    ...
    public class RuntimeManager : MonoBehaviour
    {
        ...
        {
            var callbackInfo = Marshal.PtrToStructure<FMOD.ERRORCALLBACK_INFO>(commanddata1);
            ...
            if ((callbackInfo.instancetype == FMOD.ERRORCALLBACK_INSTANCETYPE.CHANNEL ||
                ...
                && (callbackInfo.result == FMOD.RESULT.ERR_INVALID_HANDLE ||
                    callbackInfo.result == FMOD.RESULT.ERR_CHANNEL_STOLEN))
                return FMOD.RESULT.OK;
            ...
            get
            {
                ...
                if (instance == null)
                {
                    ...
                    try
                    {
                        ...
                        OpenHarmonyJSObject openHarmonyJSObject =
 new OpenHarmonyJSObject("ClassFMOD" + FMOD.VERSION.dllSuffix);
        ...
        private FMOD.RESULT Initialize()
        {
            ...
            if (fmodSettings.EnableMemoryTracking) studioInitFlags |= FMOD.Studio.INITFLAGS.MEMORY_TRACKING;
            ...
            if (currentPlatform.IsOverlayEnabled) SetOverlayPosition();
            return initResult;
        }
        ...
        private static AttachedInstance FindOrAddAttachedInstance(FMOD.Studio.EventInstance instance,
            Transform transform, FMOD.ATTRIBUTES_3D attributes)
        {
        ...
        {
            var attachedInstance = FindOrAddAttachedInstance(instance, gameObject.transform,
        ...
        {
            var attachedInstance =
                FindOrAddAttachedInstance(instance, transform, RuntimeUtils.To3DAttributes(transform, rigidBody));
        ...
        public static void AttachInstanceToGameObject(FMOD.Studio.EventInstance instance, Transform transform,
            Rigidbody2D rigidBody2D)
        {
            var attachedInstance = FindOrAddAttachedInstance(instance, transform,
 ...
 RuntimeUtils.GetCommonPlatformPath(Path.Combine(bankFolder, Settings.Instance.TargetSubFolder));
...
#endif // (UNITY_ANDROID || UNITY_WEBGL) && !UNITY_EDITOR
                {
                    ...
                    var loadResult = Instance.studioSystem.loadBankFile(bankPath, FMOD.Studio.LOAD_BANK_FLAGS.NORMAL,
                        out loadedBank.Bank);
        ...
        private static void LoadBank(TextAsset asset, bool loadSamples, string bankId)
        {
            ...
            else
            {
                ...
                var loadedBank = new LoadedBank();
                var loadResult = Instance.studioSystem.loadBankMemory(asset.bytes, FMOD.Studio.LOAD_BANK_FLAGS.NORMAL,
                    out loadedBank.Bank);
        ...
        public static FMOD.GUID PathToGUID(string path)
        {
            ...
            else
            {
                ...
                if (result == FMOD.RESULT.ERR_EVENT_NOTFOUND) throw new EventNotFoundException(path);
            }
        ...
        public static FMOD.Studio.EventDescription GetEventDescription(FMOD.GUID guid)
        {
            ...
            else
            {
                ...
                if (result != FMOD.RESULT.OK) throw new EventNotFoundException(guid);
        ...
        public static void SetListenerLocation(int listenerIndex, GameObject gameObject,
            GameObject attenuationObject = null, Vector3 velocity = new())
        {
            if (attenuationObject)
                Instance.studioSystem.setListenerAttributes(listenerIndex,
        ...
        {
            FMOD.Studio.Bus bus;
            if (StudioSystem.getBus(path, out bus) != FMOD.RESULT.OK) throw new BusNotFoundException(path);
            return bus;
        }
        ...
        {
            FMOD.Studio.VCA vca;
            if (StudioSystem.getVCA(path, out vca) != FMOD.RESULT.OK) throw new VCANotFoundException(path);
            return vca;
        }
        ...
        public static void PauseAllEvents(bool paused)
        {
            ...
            if (StudioSystem.getBus("bus:/", out masterBus) == FMOD.RESULT.OK) masterBus.setPaused(paused);
        }
